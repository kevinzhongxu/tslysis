## lysis.R
## An automated R script for estimating the taxon-specific lysis of microbe.
...
##
## lysis.R function will firstly calculate the relative abundance of each fraction of the samples. Then the lysis will be calculated by the ratio of the relative abundance of extracellular.rRNA to cellular.rRNA. 
...
lysis <- function(otu_table) {
  ...
  if(is.null(otu_table)) {
    ...
    #meg.export <- meg[, c("Taxon","sample.id","Relative.Abundance_Cells", "Relative.Abundance_Cellular.rDNA", "Relative.Abundance_Cellular.rRNA","Relative.Abundance_Extracellular.rRNA", "Lysis","Growth")]
    meg.export <- meg[, c("Taxon","sample.id","Relative.Abundance_Cellular.rDNA", "Relative.Abundance_Cellular.rRNA","Relative.Abundance_Extracellular.rRNA", "Lysis", "group")]
    
    write.table(meg.export, file = "Output_Lysis&Relative-abundance.txt", row.names = F, col.names = TRUE, sep="\t", quote = F)
    #write.csv(meg.export, file = "Output_Lysis&Relative-abundance.csv", row.names = F, col.names = TRUE, sep="\t", quote = F)
